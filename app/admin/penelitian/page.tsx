...
export default function AdminPenelitianPage() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    await supabase.from("penelitian").insert([form]);
    ...
    <main className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="text-lg font-bold text-slate-800">Kelola Penelitian</h1>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-8">
        ...
        <Card className="border border-blue-100">
          <CardContent className="p-6 space-y-4">
            <p className="text-sm font-semibold text-slate-700 flex items-center gap-2">
              <Plus className="w-4 h-4 text-blue-500" /> Tambah Penelitian
            </p>
            <input
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <p className="text-center text-slate-400 text-sm">Belum ada data penelitian.</p>
          ) : (
            data.map((item) => (
