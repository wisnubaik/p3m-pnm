...
export default function PengumumanPage() {
  ...
  useEffect(() => {
    supabase.from("pengumuman").select("*").order("tanggal", { ascending: false })
      .then(({ data }) => { setPengumuman(data ?? []); setLoading(false); });
  }, []);
    ...
    <main className="min-h-screen bg-white overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600">
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible"
          ...
          <motion.h1 variants={fadeUp} custom={1} className="text-4xl sm:text-5xl font-bold text-white mb-4">Pengumuman</motion.h1>
          <motion.p variants={fadeUp} custom={2} className="text-blue-200 max-w-xl mx-auto text-lg">Informasi terbaru seputar kegiatan P3M Politeknik Negeri Madiun</motion.p>
        </motion.div>
      </section>
