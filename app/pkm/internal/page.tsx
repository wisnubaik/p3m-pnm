...
export default function PkmInternalPage() {
  const [pkm, setPkm] = useState<Pkm[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    supabase.from("pkm").select("*").eq("jenis", "internal").order("tahun", { ascending: false })
    ...
    <main className="min-h-screen bg-white overflow-x-hidden flex flex-col">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600">
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible"
          ...
          <motion.h1 variants={fadeUp} custom={1} className="text-4xl sm:text-5xl font-bold text-white mb-4">PKM Internal</motion.h1>
          <motion.p variants={fadeUp} custom={2} className="text-blue-200 max-w-xl mx-auto text-lg">Daftar PKM internal sivitas akademika Politeknik Negeri Madiun</motion.p>
        </motion.div>
      </section>
