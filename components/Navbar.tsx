...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-white border-b border-blue-100 shadow-sm">
              ...
              item.children ? (
                ...
                >
                  <button className="flex items-center gap-1 px-3 py-2 text-sm text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded-md transition-colors">
                    {item.label}
                    <ChevronDown className={`w-3 h-3 transition-transform ${desktopDropdown === item.label ? "rotate-180" : ""}`} />
                  </button>
                  {desktopDropdown === item.label && (
                    <div className="absolute top-full left-0 mt-1 w-44 bg-white rounded-xl shadow-lg border border-blue-100 py-1 z-50">
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-blue-100 shadow-lg max-h-[80vh] overflow-y-auto">
